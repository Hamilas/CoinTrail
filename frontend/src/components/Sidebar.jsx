...
import { LayoutDashboard, Receipt, PiggyBank, Repeat, Settings, LogOut, ChevronRight, Moon, Sun, Monitor } from 'lucide-react';
...
export default function Sidebar() {
  ...
  const menuItems = [
    ...
    { path: '/recurring', name: 'Recurring', icon: <Repeat className="w-5 h-5" /> },
    ...
    <div className="w-72 h-screen fixed left-0 top-0 p-8 flex flex-col bg-background/80 backdrop-blur-2xl border-r border-border z-50">
      ...
      <div className="flex items-center gap-4 mb-12 px-2">
        <div className="relative group">
          <div className="absolute -inset-1 bg-primary rounded-xl blur opacity-25 group-hover:opacity-100 transition duration-1000 group-hover:duration-200"></div>
          <div className="relative w-12 h-12 rounded-xl overflow-hidden shadow-sm">
            <CoinLogo className="w-full h-full" id="sidebar-coin" />
          </div>
        </div>
        <div>
      ...
      <div className="mt-auto pt-6 border-t border-border space-y-4">
        <div className="flex items-center gap-3 px-2">
          <div className="w-10 h-10 rounded-full bg-primary flex items-center justify-center text-primary-foreground font-bold text-xs shadow-md">
            {user?.name?.charAt(0) || 'U'}
          </div>
          <div className="overflow-hidden">
            <p className="text-foreground font-bold text-sm truncate leading-tight">{user?.name || 'Guest User'}</p>
            <p className="text-muted text-[10px] uppercase tracking-wider font-semibold truncate leading-tight">{user?.email || 'Guest'}</p>
          </div>
        </div>
