...
import { transactionService, budgetService, categoryService, analyticsService } from '../services/api';
import { useAuth } from '../context/AuthContext';
import { Plus, TrendingUp, TrendingDown, Wallet, ArrowUpRight, ArrowDownRight, Trash2, Receipt, Calendar } from 'lucide-react';
...
export default function Dashboard() {
  const [data, setData] = useState({ transactions: [], budgets: [], monthlyTrend: [], summary: { income: 0, expense: 0, balance: 0 } });
  ...
  const fetchData = async () => {
    try {
      const [{ data: transactionsResponse }, { data: budgetsResponse }, { data: summary }] = await Promise.all([
      ...
      setData({
        ...
        monthlyTrend: summary.monthlyTrend.map(m => ({
          ...m,
        ...
        summary: {
          ...
          expense: Number(summary.totalExpense),
          balance: Number(summary.netBalance)
        }
      });
    } catch (err) {
      console.error(err);
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <motion.div variants={itemVariants} className="lg:col-span-2 glass-card p-8 relative overflow-hidden">
          <div className="flex justify-between items-center mb-10">
            <div>
              ...
              <p className="text-muted text-sm font-medium">Monthly expense trend (last 6 months)</p>
            </div>
          </div>
        ...
        <motion.div variants={itemVariants} className="glass-card p-8 h-fit">
          ...
          <button
            onClick={() => setIsBudgetModalOpen(true)}
            className="w-full mt-10 py-3.5 rounded-xl bg-primary/5 border border-primary/20 text-primary font-bold hover:bg-primary/10 transition-all text-sm uppercase tracking-widest active:scale-[0.98]"
          >
            Plan New Budget
          </button>
        </motion.div>
      ...
      <motion.div variants={itemVariants} className="glass-card p-8">
        <div className="flex justify-between items-center mb-8 border-b border-border pb-4">
          ...
          <button
            ...
            className="text-xs font-black text-primary hover:text-accent transition-colors uppercase tracking-widest"
          >
            View Statement
          </button>
        </div>
...
function SummaryCard({ title, amount, icon, color, variants }) {
  return (
    <motion.div 
      variants={variants}
    ...
    >
      ...
      <div className={`absolute top-0 right-0 w-32 h-32 ${color} opacity-[0.03] group-hover:opacity-[0.08] transition-opacity blur-3xl`} />
      
      <div className="relative z-10">
        <div className={`${color} w-16 h-16 rounded-[20px] flex items-center justify-center mb-8 shadow-lg shadow-primary/20 transform group-hover:rotate-6 transition-transform duration-500`}>
